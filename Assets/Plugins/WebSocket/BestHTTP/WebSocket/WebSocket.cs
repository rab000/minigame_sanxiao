...
#region Properties
...

        {
            ...
            this.PingFrequency = 5000;//1000
            this.CloseAfterNoMesssage = TimeSpan.FromSeconds(1000);//600
...
#region Request Callbacks
        ...
        private void OnInternalRequestCallback(HTTPRequest req, HTTPResponse resp)
        {
			...
			//UnityEngine.Debug.LogError ("[nafio-net]--->状态网络未连接，req.State:"+req.State);
            switch (req.State)
            {
            ...
            {
				if (OnError != null) {
					//UnityEngine.Debug.LogError ("[nafio-net]--->OnError1 状态网络未连接，reason:"+reason);
        ...
        private void OnInternalRequestUpgraded(HTTPRequest req, HTTPResponse resp)
        {
            ...
            {
				if (OnError != null) {
					//UnityEngine.Debug.LogError ("[nafio-net]--->OnError2 OnInternalRequestUpgraded");
					OnError(this, req.Exception);
				}
